// Import các thư viện cần thiết
import { PositionStatus } from "@/enums/position.enum";
import { PositionRequest } from "@/interfaces/position.interface";
import { Picker } from "@react-native-picker/picker";
import React from "react";
import { Controller, useForm } from "react-hook-form";
import { Button, StyleSheet, Text, TextInput, View } from "react-native";

// Định nghĩa kiểu dữ liệu cho form (bỏ trường id)
type FormData = Omit<PositionRequest, "id">;

// Định nghĩa props cho component
interface PositionFormProps {
  onSubmit: (data: FormData) => void; // Hàm xử lý khi submit form
  initialValues?: FormData; // Giá trị ban đầu của form
  submitButtonText?: string; // Text của nút submit
  isLoading?: boolean; // Trạng thái loading
}

// Component form để thêm/sửa vị trí
export default function PositionForm({
  onSubmit,
  initialValues = {
    positionName: "",
    description: "",
    positionStatus: PositionStatus.ACTIVE,
  },
  submitButtonText = "Lưu",
  isLoading = false,
}: PositionFormProps) {
  // Sử dụng react-hook-form để quản lý form
  const {
    control, // Controller cho các input
    handleSubmit, // Hàm xử lý submit
    formState: { errors }, // Lỗi validation
  } = useForm<FormData>({
    defaultValues: initialValues, // Giá trị mặc định
  });

  return (
    <View style={styles.container}>
      {/* Input tên vị trí */}
      <Text style={styles.label}>Tên vị trí</Text>
      <Controller
        control={control}
        rules={{ required: "Tên vị trí không được để trống" }}
        render={({ field: { onChange, onBlur, value } }) => (
          <TextInput
            style={styles.input}
            onBlur={onBlur}
            onChangeText={onChange}
            value={value}
            placeholder="Ví dụ: Lập trình viên"
            autoCapitalize="words"
          />
        )}
        name="positionName"
      />
      {/* Hiển thị lỗi nếu có */}
      {errors.positionName && (
        <Text style={styles.error}>{errors.positionName.message}</Text>
      )}

      {/* Input mô tả */}
      <Text style={styles.label}>Mô tả</Text>
      <Controller
        control={control}
        rules={{ required: "Mô tả không được để trống" }}
        render={({ field: { onChange, onBlur, value } }) => (
          <TextInput
            style={[styles.input, { height: 100 }]}
            onBlur={onBlur}
            onChangeText={onChange}
            value={value}
            placeholder="Nhập mô tả chi tiết"
            multiline
          />
        )}
        name="description"
      />
      {/* Hiển thị lỗi nếu có */}
      {errors.description && (
        <Text style={styles.error}>{errors.description.message}</Text>
      )}

      {/* Dropdown chọn trạng thái */}
      <Text style={styles.label}>Trạng thái</Text>
      <Controller
        control={control}
        name="positionStatus"
        render={({ field: { onChange, value } }) => (
          <View style={styles.pickerContainer}>
            <Picker selectedValue={value} onValueChange={onChange}>
              <Picker.Item
                key={PositionStatus.ACTIVE}
                label={"Đang hoạt động"}
                value={PositionStatus.ACTIVE}
              />
              <Picker.Item
                key={PositionStatus.INACTIVE}
                label={"Không hoạt động"}
                value={PositionStatus.INACTIVE}
              />
            </Picker>
          </View>
        )}
      />

      {/* Nút submit */}
      <View style={styles.buttonContainer}>
        <Button
          title={isLoading ? "Đang xử lý..." : submitButtonText}
          onPress={handleSubmit(onSubmit)}
          color="#3182CE"
          disabled={isLoading}
        />
      </View>
    </View>
  );
}

const styles = StyleSheet.create({
  container: { flex: 1, padding: 20 },
  label: { fontSize: 16, marginBottom: 5, marginTop: 15, fontWeight: "500" },
  input: {
    borderWidth: 1,
    borderColor: "#ccc",
    padding: 10,
    borderRadius: 5,
    fontSize: 16,
    textAlignVertical: "top",
  },
  pickerContainer: { borderWidth: 1, borderColor: "#ccc", borderRadius: 5 },
  error: { color: "red", marginTop: 5 },
  buttonContainer: { marginTop: 30 },
});
